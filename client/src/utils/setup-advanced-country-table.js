const SetupAdvancedCountryTable  = (country_name, areas , is_main = false) => {
    ...
    const rows = areas.map(c => {
        ...
        if(country_name == 'United States' && is_main){
            ...
            const recovered_change = yesterday && c.highest_recovered ? c.highest_recovered - yesterday.recovered : 0
            obj.hospitalized = c.highest_hospitalized
            obj.hospitalized_change = hospitalized_change
            obj.recovered = c.highest_recovered
            obj.recovered_change = recovered_change
    ...
    const headCells = [
        ...
        { id: 'confirmed_change', numeric: true, disablePadding: false, label: '24H', class: 'delta' },
        { id: 'deaths', numeric: true, disablePadding: false, label: 'Deaths', class: 'deaths' },
        { id: 'deaths_change', numeric: true, disablePadding: false, label: '24H', class: 'delta' },
    ]

    if(country_name == 'United States' && is_main){
        headCells.push(
            { id: 'hospitalized', numeric: true, disablePadding: false, label: 'Hospitalized', class: 'hospitalized' },
            { id: 'hospitalized_change', numeric: true, disablePadding: false, label: '24H', class: 'delta' },
            { id: 'recovered', numeric: true, disablePadding: false, label: 'Recovered', class: 'tests' },
            { id: 'recovered_change', numeric: true, disablePadding: false, label: '24H', class: 'delta' },
            { id: 'tests', numeric: true, disablePadding: false, label: 'Tests', class: 'tests' },
            { id: 'tests_change', numeric: true, disablePadding: false, label: '24H', class: 'delta' }
        )
    }
